...
const ConvertToWord = (value) => {
  switch (value) {
    ...
    case "8":
      return "On Hold";
    case "9":
      return "Awaiting Client Comments";
    default:
      return "Unknown";
  }
}
...
const DetermineRowFormat = props => {
  ...
  budgetStatus = (row.Actual >= (row.Budget * 0.9)) ? classes.bad : budgetStatus;
    ...
    return (
      <TableRow key={row.jmpJobID}>
        ...
        <TableCell className={classes.default} style={{width: "142px"}}>{row.Budget}</TableCell>
        <TableCell className={budgetStatus} style={{width: "142px"}}>{row.Actual}</TableCell>
...
class DataView extends Component {
  ...
  tick () {
    fetch('http://APPSERVER:8080/api/openJobs/?part=100841')
    .then(results => {
      return results.json();
    }).then(data => {
